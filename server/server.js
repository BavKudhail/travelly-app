...
const { Trip, CountryBadge, ActivityBadge, User } = require('./models');
...
app.post('/images/:id', upload.single('image'), async (req, res) => {
  ...
  const trip = await Trip.findByIdAndUpdate({ _id: req.params.id }, { imageUrl: `/images/${result.Key}` }, { new: true, runValidators: true });
...
app.post('/user/:id/image', upload.single('image'), async (req, res) => {
  ...
  console.log(result);
  //   console.log(req.body.tripId);
  const user = await User.findByIdAndUpdate({ _id: req.params.id }, { profilePicture: `/images/${result.Key}` }, { new: true, runValidators: true });
...
app.post('/badgeImages/:id', upload.single('image'), async (req, res) => {
  ...
  if (req.body.model === 'CountryBadge') {
    const countryBadge = await CountryBadge.findByIdAndUpdate({ _id: req.params.id }, { badgeImage: `/images/${result.Key}` }, { new: true, runValidators: true });
    ...
    const activityBadge = await ActivityBadge.findByIdAndUpdate({ _id: req.params.id }, { badgeImage: `/images/${result.Key}` }, { new: true, runValidators: true });
    //   TODO: Will need to add result.key to relevant model (may have to send key to front end and trigger a gql mutation from there?)
